...
- 2026-09-05 ACCEPTED MISCONFIG @ affiliates.betpanda.io/rest/*: Wildcard CORS + credentials confirmed via live OPTIONS probe reflecting arbitrary Origin with ACAC: true; 20+ authenticated endpoints including password-reset and 2FA endpoints
- 2026-09-05 ACCEPTED IDOR @ betpandacasino.io/rest/user/*: Money-flow endpoints confirmed (POST-only balances 405, settings 401); CORS pinned but server-side authZ unverified
- 2026-09-05 ACCEPTED BUSLOGIC @ cable.betpanda.io: Unauthenticated event ingestion with CORS * confirmed — injection surface validated
- 2026-09-05 REJECTED OAUTH @ betpanda.io/api/auth/authorize: 301 redirects to betpandacasino.io SPA catch-all; no server-side OAuth endpoint exists
- 2026-09-05 REJECTED MISCONFIG @ nano-public.s3.eu-west-1.amazonaws.com: S3 bucket listing returns 403 Forbidden; no enumeration possible
- 2026-09-05 REJECTED SSRF @ betpandacasino.io/rest/properties/manifest: Manifest is static JSON with hardcoded S3 URLs; no user-controllable parameters for URL manipulation
- 2026-09-05 NEW ENDPOINT @ betpandacasino.io/rest/user/authenticate: Real Spring Boot endpoint (403 JSON, not SPA catch-all). Returns 403 with dummy creds. Likely requires CAPTCHA token (x-captcha-token in CORS allow-headers).
...
- 2026-09-05 REJECTED MISCONFIG @ betpanda.partners/rest/*: CORS properly pinned — OPTIONS and GET with evil Origin return Vary:Origin but NO ACAO reflection; same Spring Boot backend as betpandacasino.io, cross-origin credential vector ABSENT (only affiliates.betpanda.io is wildcard).
- 2026-09-05 ACCEPTED BUSLOGIC @ cable.betpanda.io/cable/user-event: full schema reverse-derived (eventType/userId/registeredOn/amount/referrer/currency/ip/device/metadata); arbitrary eventType accepted incl. XSS strings; negative amounts and nested script accepted (200) — injection primitive concrete.
...
- 2026-09-05 ACCEPTED BUSLOGIC @ cable.betpanda.io/cable/user-event: Unauthenticated event ingestion with CORS * confirmed — injection surface validated
- 2026-09-06 ACCEPTED MISCONFIG @ affiliates.betpanda.io/rest/*: Wildcard CORS + credentials confirmed via live OPTIONS probe reflecting arbitrary Origin with ACAC: true; 20+ authenticated endpoints including password-reset and 2FA endpoints
- 2026-09-06 ACCEPTED IDOR @ betpandacasino.io/rest/user/*: Money-flow endpoints confirmed (POST-only balances 405, settings 401); CORS pinned but server-side authZ unverified
- 2026-09-06 ACCEPTED BUSLOGIC @ cable.betpanda.io/cable/user-event: Unauthenticated event ingestion with CORS * confirmed — injection surface validated
- 2026-09-06 REJECTED OAUTH @ betpanda.io/api/auth/authorize: 301 redirects to betpandacasino.io SPA catch-all; no server-side OAuth endpoint exists
- 2026-09-06 REJECTED MISCONFIG @ nano-public.s3.eu-west-1.amazonaws.com: S3 bucket listing returns 403 Forbidden; no enumeration possible
- 2026-09-06 REJECTED SSRF @ betpandacasino.io/rest/properties/manifest: Manifest is static JSON with hardcoded S3 URLs; no user-controllable parameters for URL manipulation
- 2026-09-06 NEW ENDPOINT @ betpandacasino.io/rest/user/authenticate: Real Spring Boot endpoint (403 JSON, not SPA catch-all). Returns 403 with dummy creds. Likely requires CAPTCHA token (x-captcha-token in CORS allow-headers).
- 2026-09-06 AUTH MODEL @ betpandacasino.io: REFRESH_TOKEN cookie (HttpOnly, SameSite=Lax, Secure, Path=/rest/user/refresh) confirmed via logout. SameSite=Lax limits cross-origin cookie sending.
- 2026-09-06 NEW ENDPOINTS @ affiliates.betpanda.io: JS reveals /agent/set-deposit-withdraw-limit (POST, financial), /payouts/single-currency-list, /reports/commission, /reports/details, /reports/sub-affiliates, /rest/user/password/reset, /rest/user/set-2fa-setting, /rest/user/metrics/affiliate, /rest/agent/list, /rest/agent/create, /rest/agent/events/list, /rest/v2/
- 2026-09-06 REJECTED MISCONFIG @ affiliates.betpanda.io/actuator + api-docs: Spring Boot actuator/OpenAPI not publicly exposed (SPA catch-all).
- 2026-09-06 NEW ASSET @ betpanda.partners: Dedicated in-scope host, "Betpanda" casino brand SPA fronting SAME Spring Boot `/rest` backend as betpandacasino.io
- 2026-09-06 NEW ENDPOINT @ affiliates.betpanda.io/rest/public/config: Unauth Spring Boot route (200 JSON) leaks operatorId=1, strapiApiUrl=/cms, CloudFront d3ec3n7kizfkuy.cloudfront.net, linkUrl=betpanda.partners, support@betpanda.io
- 2026-09-06 ACCEPTED MISCONFIG @ affiliates.betpanda.io/rest/public/*: Wildcard CORS + credentials (ACAO:<any Origin> + ACAC:true) confirmed on the unauthenticated public layer via live GET /rest/public/config with evil Origin returning reflected ACAO (200, no auth) — backend-wide flaw, not limited to authed routes.
- 2026-09-06 REJECTED BUSLOGIC @ affiliates.betpanda.io/rest/public/recover-password/email/{email}: Email-enumeration oracle is explicitly OUT of scope; no novel logic reachable without a test account. Not pursued.
- 2026-09-06 REJECTED MISCONFIG @ betpanda.partners/rest/*: CORS properly pinned — OPTIONS and GET with evil Origin return Vary:Origin but NO ACAO reflection; same Spring Boot backend as betpandacasino.io, cross-origin credential vector ABSENT (only affiliates.betpanda.io is wildcard).
- 2026-09-06 ACCEPTED BUSLOGIC @ cable.betpanda.io/cable/user-event: full schema reverse-derived (eventType/userId/registeredOn/amount/referrer/currency/ip/device/metadata); arbitrary eventType accepted incl. XSS strings; negative amounts and nested script accepted (200) — injection primitive concrete.
- 2026-09-06 NEW ENDPOINTS @ affiliates.betpanda.io: /rest/v2/report (POST-only 405-GET), /rest/v2/report/sub-affiliates, /rest/v2/report/daily-stats-with-comparison, /rest/user/selectable-payout-currencies, /rest/user/selectable-payout-networks, /rest/metrics/top-metrics-revenue-chart, /rest/trk/list, /rest/agent/id/{id}, /rest/agent/enable/{id}/{enabled} (GET state-changing, 401).
- 2026-09-06 NEW SPA ROUTE @ affiliates.betpanda.io: /reset-password/:affiliateId/:resetPasswordCode — reset code in URL is the credential binding for POST /rest/user/password/reset.
- 2026-09-06 NEW ENDPOINTS @ affiliates.betpanda.io/rest/public/phone/*: signin/verify + register/verify POST-only JSON (415 on form-encoded ⇒ handler present), reflect evil Origin + ACAC:true; JSON body returns 404 (schema obfuscated).
- 2026-09-06 ACCEPTED MISCONFIG @ affiliates.betpanda.io/rest/*: Wildcard CORS + credentials confirmed via live OPTIONS probe reflecting arbitrary Origin with ACAC: true; 20+ authenticated endpoints including password-reset and 2FA endpoints
- 2026-09-06 ACCEPTED IDOR @ betpandacasino.io/rest/user/*: Money-flow endpoints confirmed (POST-only balances 405, settings 401); CORS pinned but server-side authZ unverified
- 2026-09-06 ACCEPTED BUSLOGIC @ cable.betpanda.io/cable/user-event: Unauthenticated event ingestion with CORS * confirmed — injection surface validated
- 2026-09-06 REJECTED OAUTH @ betpanda.io/api/auth/authorize: 301 redirects to betpandacasino.io SPA catch-all; no server-side OAuth endpoint exists
...
- 2026-09-06 NEW SPA ROUTE @ affiliates.betpanda.io: /reset-password/:affiliateId/:resetPasswordCode — reset code in URL is the credential binding for POST /rest/user/password/reset.
- 2026-09-06 NEW ENDPOINTS @ affiliates.betpanda.io/rest/public/phone/*: signin/verify + register/verify POST-only JSON (415 on form-encoded ⇒ handler present), reflect evil Origin + ACAC:true; JSON body returns 404 (schema obfuscated).
- 2026-09-06 ACCEPTED MISCONFIG @ affiliates.betpanda.io/rest/*: Wildcard CORS + credentials re-confirmed this cycle (bundle unchanged main.ef021e68.js; password/reset, set-2fa-setting, change-password, set-profile all 405-GET reflecting evil Origin + ACAC:true). Flagship finding, POC still auth-gated.
- 2026-09-06 ACCEPTED IDOR @ betpandacasino.io/rest/user/*: Money-flow endpoints re-confirmed; CORS pinned; server-side authZ unverified → BOLA stands.
- 2026-09-06 ACCEPTED BUSLOGIC @ cable.betpanda.io/cable/user-event: Unauth event ingestion re-confirmed (405/200 baseline stable).
...
- 2026-09-07 ACCEPTED MISCONFIG @ affiliates.betpanda.io/rest/*: Fresh live OPTIONS /rest/user/password/reset with Origin https://evil.example → 200 ACAO:https://evil.example + ACAC:true + allow-methods GET,POST,OPTIONS,PUT,HEAD,DELETE. Bundle unchanged (main.ef021e68.js). Flagship stands, POC auth-gated.
- 2026-09-07 ACCEPTED IDOR @ betpandacasino.io+betpanda.partners App B /rest/user/*: Full handler map derived (authenticate POST 403, settings 401 "No http-session found", password/reset 405, refresh 405, account-balances-and-bonuses 405, zendesk/jwt 405, properties/manifest 200); unknown /rest/* = JSON 404 baseline. Cross-brand single-account session test remains cheapest proof. CORS pinned both hosts.
- 2026-09-07 ACCEPTED BUSLOGIC @ cable.betpanda.io/cable/user-event: Baseline stable (405-GET/200-POST); at passive ceiling.
- 2026-09-07 REJECTED MISCONFIG @ betpandacasino.io/rest namespace breadth: 24-path GET sweep (deposit/withdraw/wallet/cashier/transaction/payments/kyc/games/... all 404 JSON) — App B exposes only user+properties handlers; no hidden money-flow namespaces.
- 2026-09-07 REJECTED MISCONFIG @ betpandacasino.io+betpanda.partners/cms: Strapi closed — no public content types, no /cms/graphql, no users-permissions routes (all JSON 404); affiliates /cms = SPA catch-all; cable /cms = Express 404 shell. CMS content exposure eliminated on every host.
- 2026-09-07 NEW ENDPOINT @ betpandacasino.io/rest/user/zendesk/jwt: Real Spring handler (405 POST-only) — Zendesk SSO JWT mint on shared App B; also confirms /rest/user/password/reset exists on App B (was affiliates-only). PARKED: auth-gated, schema unknown, no passive proof path.
- 2026-09-07 ACCEPTED MISCONFIG @ affiliates.betpanda.io/rest/*: Wildcard CORS + credentials re-confirmed fresh (config 200 ACAO-reflect+ACAC:true, password/reset OPTIONS 200 ACAO-reflect+ACAC:true); bundle unchanged main.ef021e68.js; POC remains auth-gated.
...
- 2026-09-08 REJECTED MISCONFIG @ betpandacasino.io/rest/user/account-balances-and-bonuses: OPTIONS returns generic 200 CORS envelope (ACAO own-host + ACAC:true + full allow-methods) **identical** to /rest/user/notreal123 404 control — OPTIONS cannot distinguish real handler from catch-all on App B; no new reachable handler confirmed.
- 2026-09-08 ACCEPTED MISCONFIG @ affiliates.betpanda.io/rest/*: Wildcard CORS+credentials re-verified live this cycle (GET /rest/public/config 200, ACAO:https://evil.example reflected + ACAC:true, no auth). Bundle unchanged main.ef021e68.js; flagship POC remains auth-gated.
- 2026-09-08 ACCEPTED IDOR @ betpandacasino.io+betpanda.partners: Shared backend re-confirmed; CORS pinned on both; single-account cross-brand session test remains cheapest de-gate.
- 2026-09-08 REJECTED MISCONFIG @ betpandacasino.io/rest/user/account-balances-and-bonuses: OPTIONS returns generic 200 CORS envelope (ACAO own-host + ACAC:true + full allow-methods) identical to /rest/user/notreal123 404 control — OPTIONS cannot distinguish real handler from catch-all on App B; no new reachable handler confirmed.
- 2026-09-08 ACCEPTED MISCONFIG @ betpanda.partners+betpandacasino.io/rest/user/refresh: OPTIONS envelope parity confirmed — identical allow-headers/methods (incl x-captcha-token, x-site-name-id, x-maintenance-reason, x-preferred-app-context); shared App B handler set on both brands, supports credentialed cross-brand test.
- 2026-09-08 ACCEPTED MISCONFIG @ betpandacasino.io/rest/user/refresh: CORS re-confirmed pinned (ACAO own-host + ACAC:true); __cflb SameSite=None;Secure is LB-affinity only, no auth value — cross-origin read still blocked.
- 2026-09-08 REJECTED MISCONFIG @ betpandacasino.io x-site-name-id: forged client header ignored in CORS envelope (still betpandacasino_io) — tenant discriminator is host-derived; no client-controlled tenant switch at filter layer.
